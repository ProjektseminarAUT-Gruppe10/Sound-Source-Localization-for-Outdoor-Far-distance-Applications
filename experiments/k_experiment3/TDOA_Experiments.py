...
for dis in distances:
    for ang in angles:
        ...
        for n in range(0,N):
            ...
            signalA = signals[0]
            signalB = signals[1]
            signalAF = butterWorthFilter(signalA, meta["sampling_rate"], 2500)
            signalBF = butterWorthFilter(signalB, meta["sampling_rate"], 2500)
            
            TDOA_CSOM, t, csom = tdoa_csom(signalAF, signalBF, fs=meta["sampling_rate"], window=1000)
        ...
        print(dis, ";", ang, ";", source_pos, ";", np.average(SNRls) , ";", np.std(SNRls), ";", np.average(TDOA_realls) , ";", np.std(TDOA_realls), ";", np.average(TDOA_csomls) , ";", np.std(TDOA_csomls))
